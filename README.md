...
Project Goal:
	Our goal is to implement a program that will be able to predict whether a stock chart (with 50-80 candles) has formed a potential Head and Shoulder pattern, a common stock trading pattern, in the stock candle images. Instead of using prices to see if the rolling means converge or not, we use the actual price graphs for training and classification. 
...
- We needed to engineer the data from scratch by selecting the timeframe where there is a head and shoulder pattern in the stock chart. Since there are no Head and Shoulder Stock Pattern datasets online, we needed a dataset full of hundreds, if not thousands of images of the Head and Shoulder Pattern. We collected raw stock/asset data from FXCM's and Oanda's API and using extrenal services from TradingView. Additionally, we implemented functions that will detect and recognize head and shoulder patterns in our time series data. We have two different parameters (tolerance and rolling window) to recognize head and shoulder patterns in our data. After using our functions on the data frames, we got a list of ranges of time that include head and shoulder patterns during those time intervals. Then, we draw them out with matplotlib (see below). After getting the images that have potential head and shoulder pattern, we might hand label them with 1 or 0 (some of the graphs are not really head and shoulder) for classification training.
